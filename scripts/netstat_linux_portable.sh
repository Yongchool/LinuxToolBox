...
usage() {
  echo "Usage: $0 [-d DEV]... [-i INTERVAL] [-o OUT_DIR] [TOTAL_SECONDS]"
}
...
if [[ -n "$INTERVAL" ]]; then
  ...
  uint "$INTERVAL" || { err "PERFPMR_MONITOR_INTVLTIME must be integer"; exit 1; }
else
  if [[ -z "$TOTAL" || "$TOTAL" -lt 601 ]]; then
    INTERVAL=10
  else
    INTERVAL=60
  fi
...
TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" netstat_portable.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
collect_sock() {
  ...
  echo
  echo "ss -tan state summary"
  echo "---------------------"
  if command -v ss >/dev/null 2>&1; then
    ss -tan 2>/dev/null | awk 'NR>1 { s[$1]++ } END { for (k in s) print k, s[k] }' | sort || true
  fi
}
...
OUTFILE="$OUT_DIR/netstat.int"
SUMFILE="$OUT_DIR/netstat.sum"
METAFILE="$OUT_DIR/netstat.meta"
...
if [[ -n "$TOTAL" && "$TOTAL" -gt 0 ]]; then
  ...
  for ((n=1; n<=it; n++)); do
    echo >> "$OUTFILE"
    echo "===== Interval sample $n/$it =====" >> "$OUTFILE"
    echo "Sample time: $(date '+%F %T %Z')" >> "$OUTFILE"
    collect_sock >> "$OUTFILE"
    echo >> "$OUTFILE"
    echo "/proc/net/dev (sample)" >> "$OUTFILE"
    cat /proc/net/dev >> "$OUTFILE" 2>/dev/null || true
    (( n < it )) && sleep "$INTERVAL"
  done
...
END {
  print "Socket state counts"
  print "-------------------"
  for (k in st) print k, st[k]

  print ""
  print "Interface RX/TX bytes roll-up"
  print "------------------------------"
